function Dashboard({ stats, onReset }) {
  const practiceMinutes = Math.floor(stats.totalPracticeSeconds / 60);

  const practiceSeconds = Math.floor(stats.totalPracticeSeconds % 60);
      ...
      {!stats.recentTests || stats.recentTests.length === 0 ? (
        ...
        <ul className="recent-tests">
          {stats.recentTests.map((test, index) => (
            <li key={index}>
              {test.wpm} WPM • {test.accuracy}% • {test.mode}
            </li>
          ))}
        </ul>
      )}
