...
import { AlertTriangle, Check, Clock, Globe, Mail, Search, Swords, Trash2, User, UserPlus, Users, X } from "lucide-react";
...
export default function Friends({
  ...
  const findQuickMatch = async () => {
    ...
    try {
      await fetch(
        `https://ambarmishradb.onrender.com/api/challenges/matchmake/join?username=${user.name}`,
        {
          method: "POST",
        }
      );
  ...
  const cancelSearch = async () => {
    try {
      await fetch(
        `https://ambarmishradb.onrender.com/api/challenges/matchmake/leave?username=${user.name}`,
        {
          method: "POST",
        }
      );
    ...
    const fetchChallenges = async () => {
      try {
        const res = await fetch(
          `https://ambarmishradb.onrender.com/api/challenges/${user.name}/pending`
        );
  ...
  const handleChallengeResponse = async (challengeId, status) => {
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/challenges/${challengeId}/status?status=${status}`,
        {
          method: "PUT",
        }
      );
  ...
  const sendChallenge = async (receiverName, duration = 30) => {
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/challenges/send?sender=${user.name}&receiver=${receiverName}&duration=${duration}`,
        {
          method: "POST",
        }
      );
      ...
      if (res.ok) {
        alert(
          `Challenge sent to ${receiverName}! Waiting for them to accept.`
        );
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/users/search?query=${searchQuery}`
      );

      if (res.ok) {
        setSearchResults(
          (await res.json()).filter((u) => u.name !== user.name)
        );
      }
    } catch (err) {
  ...
  const sendRequest = async (friendName) => {
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/users/${user.name}/request-friend/${friendName}`,
        {
          method: "POST",
        }
      );
  ...
  const acceptRequest = async (reqName) => {
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/users/${user.name}/accept-friend/${reqName}`,
        {
          method: "POST",
        }
      );
  ...
  const rejectRequest = async (reqName) => {
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/users/${user.name}/reject-friend/${reqName}`,
        {
          method: "POST",
        }
      );
  ...
  const confirmRemoveFriend = async () => {
    ...
    try {
      const res = await fetch(
        `https://ambarmishradb.onrender.com/api/users/${user.name}/remove-friend/${friendToRemove}`,
        {
          method: "POST",
        }
      );
  ...
  if (!user) {
    return (
      <div className="friends-container">
        ...
        >
          Please log in to manage friends!
        </h2>
      </div>
    );
  }
  ...
  return (
    <div className="friends-container" style={styles.container}>
      <style>{`
        ...
        .action-icon-btn.challenge:hover {
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            ...
            <div style={styles.modalActions}>
              ...
              <button
                onClick={() =>
                  sendChallenge(challengeTarget, challengeTime)
                }
      ...
      <div style={styles.grid}>
        ...
        >
          ...
          >
            ...
            >
              Play a random 30-second duel against anyone online!
            </p>
          ...
          {pendingChallenges.length > 0 && (
            ...
            >
              ...
              >
                <Swords size={20} /> Match Challenges (
                ...
                {pendingChallenges.map((challenge) => (
                  ...
                  >
                    ...
                    >
                      <ZoomableAvatar
                        avatarUrl={
                          challenge.senderAvatar || challenge.avatar
                        }
                        name={
                    ...
                    >
                      <button
                        ...
                        className="action-icon-btn accept"
                        title="Accept"
                      >
                        <Check size={18} />
                      </button>
            ...
            {myFriends.length === 0 ? (
              ...
              <div style={styles.list}>
                {myFriends.map((friend, idx) => {
                  ...
                  // Use the backend avatar first, then local storage.
                  const friendAvatar =
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <button
                          onClick={() =>
                            setChallengeTarget(friendName)
                          }
                          className="action-icon-btn challenge"
                          title="Challenge"
                        >
                          <Swords size={18} />
                        </button>
        ...
        <div style={styles.card}>
          ...
          {loading && (
            <p style={styles.emptyStateText}>Searching...</p>
          )}
                ...
                >
                  ...
                  >
                    ...
                    <span
                      ...
                      onClick={() =>
                        setSelectedFriendProfile(result.name)
                      }
                    >
                      {result.name}
                    </span>
                  ...
                  {isFriend ? (
                    ...
                    <button
                      onClick={() => sendRequest(result.name)}
                      style={styles.addBtn}
                    >
