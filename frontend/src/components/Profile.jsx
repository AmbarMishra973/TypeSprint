...
export default function Profile({ user, stats, clearStats }) {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        try {
          await fetch(
            "https://ambarmishradb.onrender.com/api/users/update-avatar",
            {
              method: "POST",
              headers: {
  ...
  const todaysStats = useMemo(() => {
    ...
    const totalSeconds = todaysTests.reduce(
      (sum, t) => sum + (t.time || 0),
      0
    );
    ...
    const avgWpm = todaysTests.length
      ? Math.round(
          todaysTests.reduce((sum, t) => sum + t.wpm, 0) /
            todaysTests.length
        )
      : 0;

    const topAcc = Math.max(
      0,
      ...todaysTests.map((t) => t.accuracy)
    );

    const avgAcc = todaysTests.length
      ? Math.round(
          todaysTests.reduce((sum, t) => sum + t.accuracy, 0) /
            todaysTests.length
        )
      : 0;
  ...
  // Practice calendar state and navigation
  ...
  const missedKeysArray = Object.entries(
    stats?.globalMissedKeys || {}
  )
    .map(([key, count]) => ({
      key,
      count,
  ...
  const maxMisses =
    missedKeysArray.length > 0 ? missedKeysArray[0].count : 1;
    ...
    <div className="profile-container">
        ...
        <div className="profile-user-info">
          ...
          >
            <div
              ...
              onClick={() =>
                avatarPreview && setIsAvatarFullscreen(true)
              }
              title={
                avatarPreview
      ...
      <section className="profile-section">
        ...
        <div className="profile-stats-grid">
          ...
          <StatCard
            label="Top Speed"
            value={`${allTime.topSpeed} wpm`}
          />
          <StatCard
            label="Average Speed"
            value={`${allTime.avgSpeed} wpm`}
          />
          <StatCard
            label="Top Accuracy"
            value={`${allTime.topAccuracy}%`}
          />
          <StatCard
            label="Avg Accuracy"
            value={`${allTime.avgAccuracy}%`}
          />
        </div>
      </section>
              ...
              item.empty ? (
                ...
                <div
                  ...
                  style={{
                    ...
                    borderRadius: "6px",
                    fontSize: "0.9rem",
                    fontWeight: item.hasPracticed
                      ? "bold"
                      : "normal",
                    backgroundColor: item.hasPracticed
                      ? "var(--accent-color)"
                      : "rgba(255,255,255,0.03)",
                    color: item.hasPracticed
                      ? "#000"
                      : "var(--text-primary)",
                    boxShadow: item.hasPracticed
      ...
      {/* Relative stats and multiplayer */}
      <section
      ...
      >
        ...
        <div
          className="profile-tab-content-card"
          style={{
            marginTop: "20px",
            background: "var(--bg-secondary)",
            padding: "25px",
        ...
        >
          {activeTab === "speed" && (
            <div>
              ...
              >
                Your all-time average speed distribution
              </h3>
              ...
              >
                This is a histogram of typing speeds, with your
                active performance tier highlighted.
              </p>
          ...
          {activeTab === "accuracy" && (
            <div>
              ...
              >
                Your all-time average accuracy distribution
              </h3>
              ...
              >
                This is a histogram of typing accuracies, with
                your active performance tier highlighted.
              </p>

              <RelativeHistogram
                data={
                  stats?.recentTests?.map((t) => t.accuracy) || []
                }
          ...
          {activeTab === "multiplayer" && (
            ...
            >
              Multiplayer analytics summary and duels history
              records load here.
            </div>
          )}
        </div>
          ...
          {missedKeysArray.length === 0 ? (
            ...
            <div className="missed-letters-layout">
              <div className="missed-letters-bars">
                {missedKeysArray.slice(0, 8).map((item) => {
                  const fillPercentage =
                    (item.count / maxMisses) * 100;

                  return (
                    <div
                      key={item.key}
                      className="error-bar-row"
                    >
              ...
              <div className="missed-letters-heatmap">
                ...
                >
                  <KeyboardHeatmap
                    missedKeys={stats?.globalMissedKeys || {}}
                  />
                </div>
              </div>
            </div>
      ...
      >
        ...
        >
          ...
          stats.recentTests.length === 0 ? (
            ...
            >
              No tests completed yet. Complete a test to see
              your log!
            </p>
          ) : (
