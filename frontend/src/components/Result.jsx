...
function Result({
  ...
  // Max WPM rounded up to a clean number for the axis
  ...
  const getX = (index) =>
    padLeft +
    (index / Math.max(safeHistory.length - 1, 1)) * innerWidth;

  const getY = (value) =>
    padTop + innerHeight - (value / roundedMaxWpm) * innerHeight;

  // Generate the polyline paths
  const points = safeHistory
    .map(
      (point, index) =>
        `${getX(index)},${getY(Number(point.wpm))}`
    )
    .join(" ");
    ...
    <div className="result-screen">
      ...
      <div className="chart-container">
        ...
        >
          ...
          {[0, Math.round(roundedMaxWpm / 2), roundedMaxWpm].map(
            (value, index) => (
              <g key={`y-axis-${index}`}>
                <text
                  x={padLeft - 10}
        ...
        <div className="slowest-words-container">
          ...
          <div className="slow-words-list">
            {slowestWords.map((item, index) => (
              <div
                key={index}
                className="slow-word-card"
              >
                <span className="slow-word-text">
                  {item.word}
                </span>

                <span className="slow-word-time">
