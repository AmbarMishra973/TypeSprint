...
function useTypingEngine() {
    ...
    function saveCurrentTest(elapsed) {
        ...
        const updatedStats = {
            ...
            averageWpm: Math.round(
                (stats.averageWpm * stats.totalTests + currentWpm) / (stats.totalTests + 1)
            ),
            highestAccuracy: Math.max(stats.highestAccuracy, accuracy),
            totalWords: stats.totalWords + correctWords + wrongWords,
            totalCharacters: stats.totalCharacters + correctCharacters + incorrectCharacters,
    ...
    function newTest() {
        setWords(testMode === "words" ? generateWords(wordLimit) : generateWords(300));
        resetTest();
        setIsRepeat(false);
    }
